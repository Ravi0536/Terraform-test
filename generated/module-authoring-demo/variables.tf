...
variable "stream_view_type" {
  description = "Information written to the stream when items are modified. Valid values: KEYS_ONLY, NEW_IMAGE, OLD_IMAGE, NEW_AND_OLD_IMAGES."
  type        = string
  default     = null
...
variable "server_side_encryption" {
  description = "Server-side encryption configuration. Set enabled to true to activate, and optionally specify a KMS key ARN."
  type = object({
    enabled     = bool
...
variable "ttl" {
  description = "TTL configuration. Set enabled to true and supply the attribute_name of the TTL attribute."
  type = object({
    enabled        = bool
...
variable "attributes" {
  description = "List of attribute definitions for hash/range/index keys. Each entry must supply name (string) and type (S, N, or B)."
  type = list(object({
    name = string
    type = string
  }))
...
variable "local_secondary_indexes" {
  description = "List of Local Secondary Index configurations. Can only be set at table creation."
  type = list(object({
    name               = string
    range_key          = string
    projection_type    = string
